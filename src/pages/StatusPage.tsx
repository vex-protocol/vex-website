        ...
        <dl className="mt-2.5 space-y-2 text-xs">
          ...
          <div className="flex items-baseline justify-between gap-3">
            ...
            <dd className="font-mono tabular-nums text-zinc-200">
              ...
              {gap > 0 ? (
                <span className="text-zinc-500"> (+{gap} unknown)</span>
              ) : null}
            </dd>
          </div>
...
function UptimeReliabilityStripCell(props: {
      ...
      <span className={`flex ${barFrame} overflow-hidden`}>
        {orderedRuns.map((run, i) => (
          <span
            key={i}
            className={
              run.ok ? "min-w-0 bg-emerald-400/90" : "min-w-0 bg-red-400/90"
            }
            style={{ flex: run.count }}
          />
        ))}
        {tailGap > 0 ? (
          <span className="min-w-0 bg-zinc-600/80" style={{ flex: tailGap }} />
        ) : null}
      </span>
    );
  }
  ...
  return (
    <span className={`flex ${barFrame} overflow-hidden`}>
      {online > 0 ? (
        <span className="min-w-0 bg-emerald-400/90" style={{ flex: online }} />
      ) : null}
      {offline > 0 ? (
        <span className="min-w-0 bg-red-400/90" style={{ flex: offline }} />
      ) : null}
      {gap > 0 ? (
        <span className="min-w-0 bg-zinc-600/80" style={{ flex: gap }} />
      ) : null}
    </span>
  );
}
...
function BuildCommitPill(props: {
  ...
  return (
    ...
    >
      ...
      <span className="shrink-0 text-[0.625rem] text-zinc-400/80">{label}</span>
...
export function StatusPage(_: { path?: string }): JSX.Element {
      ...
      try {
        ...
        if (!cancelled) {
          ...
          setSpireMeta((prev) => ({
            ...
            buildUpdatedAt: latestRun?.updated_at ?? prev?.buildUpdatedAt ?? "",
            buildUrl: latestRun?.html_url ?? prev?.buildUrl ?? "",
            healthVersion: latestSample?.serviceVersion ?? null,
            healthCheckDurationMs: latestSample?.statusCheckDurationMs ?? null,
            ...
            latestCommit: latestCommit
              ? {
                  ...
                  authorAvatarUrl: latestCommit.author?.avatar_url ?? null,
                  authorUrl: latestCommit.author?.html_url ?? null,
                }
  ...
  const totalUptimeBlockSlots = Math.max(
    1,
    Math.ceil((UPTIME_BLOCK_WINDOW_HOURS * 60) / UPTIME_BLOCK_BUCKET_MINUTES)
  );
  ...
  const runtimeRef =
    spireMeta?.runtimeCommitSha ?? spireMeta?.latestCommit?.sha ?? "unknown";
              ...
              <div className="rounded-lg border border-white/10 bg-white/[0.03] p-3">
                ...
                <p className="mt-1 font-mono text-lg text-zinc-100">
                  {spireMeta?.averageLatencyMs !== null
                    ? `${Math.round(spireMeta?.averageLatencyMs ?? 0)}ms`
                    : "n/a"}
                </p>
              </div>
            </div>
            <div className="mt-4">
              ...
              <div className="relative z-20 flex w-full gap-0.5 overflow-visible pb-1 sm:gap-[0.1875rem]">
                {paddedUptimeBlocks.map((block, index) => {
                  const slotClass = "h-6 min-w-[0.3125rem] flex-1 basis-0";
                  if (!block) {
                    return (
                      <div key={`uptime-empty-${index}`} className={slotClass}>
                  ...
                  return (
                    <div
                      ...
                      className={slotClass}
                    >
                      <UptimeReliabilityStripBlock block={block} />
                    </div>
                  );
                })}
              </div>
