...
const CACHE_KEY = "public:libvex-github:v1";
const TTL_MS = 90_000;
    ...
    try {
        const body = await cachedJson(CACHE_KEY, TTL_MS, async () => {
            const [runs, commits] = await Promise.all([
                fetchGithubApiJson("repos/vex-protocol/libvex-js/actions/runs", {
                    per_page: "1",
                }),
                fetchGithubApiJson("repos/vex-protocol/libvex-js/commits", {
                    per_page: "1",
                }),
            ]);
            return { runs, commits };
        ...
        console.error("libvex-github", err);
